...
def generate_graph():
    try:
        ...
        eall = [(u, v) for (u, v, d) in G.edges(data=True) if (u, v) not in primlist(WL, x)]
        eshort = [(u, v) for (u, v, d) in G.edges(data=True) if (u, v) in primlist(WL, x)]
        ...
        # Draw edges
        nx.draw_networkx_edges(G, pos, edgelist=eall, width=6, label="Remaining Edges")
        nx.draw_networkx_edges(
            G, pos, edgelist=eshort, width=6, alpha=0.5, edge_color="b", style="dashed",label="Shortest Path"
        )
        ...
        plt.legend()
        plt.title("Shortest Path Graph",fontsize=22)
        plt.axis("off")
        plt.tight_layout()
